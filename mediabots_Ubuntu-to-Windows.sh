...
link1_status=$(curl -Is https://unduh.mrgunhost.workers.dev/0:/ISO/2012r2.ISO | grep HTTP | cut -f2 -d" " | head -1)
link2_status=$(curl -Is https://unduh.mrgunhost.workers.dev/0:/ISO/2012r2.ISO | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://archive.org/download/WS2012R2/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO https://unduh.mrgunhost.workers.dev/0:/ISO/2012r2.ISO 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots https://unduh.mrgunhost.workers.dev/0:/ISO/2012r2.ISO
else
...
sudo wget -P /virtio https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/archive-virtio/virtio-win-0.1.215-2/virtio-win-0.1.215.iso
